...
export const drawEVMarkers = (map, evStations, zoomValue, filters) => {
  ...
  if (zoomValue < 14) {
    // Для низкого зума создаем кластеры
    ...
    // Для высокого зума создаем отдельные маркеры для каждой станции
    evStations.forEach((station) => {
      ...
      activeEVMarkers.push(marker);  // Добавляем маркер в массив активных маркеров
    });
  }
};
...
export const drawParkingMarkers = (map, parkingData, zoomValue, filters) => {
  ...
  if (Object.keys(filters).length < 1) {
    console.log('No filters applied: ', parkingData);
  } else {
    console.log('Filters applied: ', filters);
    parkingData = filterParkingData(parkingData, filters);
  }
