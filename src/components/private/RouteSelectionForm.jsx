...
const RouteSelectionForm = ({
  ...
  const [routeSelected, setRouteSelected] = useState(false);  // Для отслеживания, был ли выбран маршрут
  ...
  const fetchTravelTime = async () => {
    try {
      const startCoordinates = startPointQuery.split(",").map(Number);
      const finishCoordinates = finishPointQuery.split(",").map(Number);

      const url = `https://api.mapbox.com/directions/v5/mapbox/driving/${startCoordinates.join(',')};${finishCoordinates.join(',')}?access_token=${apiKey}`;
